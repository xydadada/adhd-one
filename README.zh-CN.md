...
- Windows Server 2025 CI：run `31823906216` 在安装版通过 12/13 个循环后失败；随后 run `31827138503` 在打包前因 evidence verifier 的 Windows 路径规范化测试失败。hardened 验证等待新的完整运行；Server 2025 结果不能作为 Windows 11 证据。
...
- 当前本地证据：`npm run check` 通过 26 个 test files（278 个通过，1 个 Windows 8.3 alias 回归因本卷没有独立短路径而跳过），JavaScript 语法门通过 21 个文件，`npm run test:doctor` 通过（20/20），真实 `npm run smoke:runtime-staging` 输出 `RUNTIME_STAGING_OK slot=A version=0.1.0-rc.6`。Setup 为 144.04 MiB；打包后二进制正常退出 3/3、force-kill 1/1、workspace-write 1/1、真实 Portable 模式 1/1 均通过且无残留 PID。workspace 证据确认使用包内 ASAR RPC、两轮 Provider、PowerShell 执行和 Session 归档。这些结果仍不代表 Windows 11、Stable、性能或完整安装后 Release E2E 已完成。
